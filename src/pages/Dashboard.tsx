...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (searchQuery.trim() === "") {
      ...
      const filtered = secrets.filter(
        (secret) =>
          (secret.title?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false) ||
  ...
  const handleAddSecret = async (newSecret: Secret) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Erreur lors de l'ajout du secret")
    }
  }
  ...
  const handleEditSecret = async (updatedSecret: Secret) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/secrets/${updatedSecret.id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-4xl p-4">
        ...
        <SecretList secrets={filteredSecrets} onSecretClick={handleSecretClick} />

        <AddSecret isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onAdd={handleAddSecret} />
